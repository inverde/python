...
# Disallow the use of number at the beginning of email
import re
""" Regular Expressions:
...
def validate_email():
    ...
    Disallow the use of numbers at the beginning [^d]
    ...
    if re.search(r"^([^\d][a-zA-Z][a-zA-Z0-9]+([a-zA-Z]\.)?){8,15}@(.+)\.(com|edu|org|gob|gov)$",
                email, re.IGNORECASE):
        return True
    else:
        return False
